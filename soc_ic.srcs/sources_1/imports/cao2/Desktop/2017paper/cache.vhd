...
architecture rtl of l1_cache is
	...
	--529-512 tag
	--511-0 data
	type rom_type is array (natural(2 ** 14 - 1) downto 0) of std_logic_vector(547 downto 0);
...
--	signal snp_wt_ack : std_logic;
	...
	signal writereq : BMSG_T;
	signal writeack : std_logic_vector;

	signal readreq1, readreq2, readreq3                                                             : MSG_T;
	signal writereq1, writereq2, writereq3, writereq4, writereq5, writereq6                         : cacheline;
...
begin
	...
	writereq_arbiter : entity work.b_arbiter6(rtl)
		port map(
			clock => Clock,
			reset => reset,
			din1  => twritereq1,
	...
	cpu_req_p : process(reset, clock)
		...
		variable tmp     : MSG_T;
		variable snpreq  : MSG_T;
		variable tmp_req: MSG_T;
		variable tmp_cal: cacheline;
					...
					if snp_res_i.adr = snpreq.adr and (snp_res_i.tag = CPU0_TAG or snp_res_i.tag = CPU1_TAG) then
						if snp_hit_i = '1' then
							...
							if snp_res_i.cmd=WRITE_CMD then
								tmp_front := "111111111111111111"&snp_res_i(31 downto 14);
							else
								tmp_front := "001111111111111111"&snp_res_i(31 downto 14);
							end if;
							-- the data inside snp-res-i need to be slightly changed, as the exclusive, need to change
							writereq3 <= (snp_res_i.val, snp_res_i.cmd, snp_res_i.tag, snp_res_i.id, snp_res_i.adr,snp_res_i.dat,tmp_front);
							tmp       := snp_res_i;
						else
							bus_req_o <= snp_res_i;
							st        := 0;
			...
			if state = 0 then           -- wait_fifo
				...
				if readres.val = '1' then
					...
					if readres.cmd = WRITE_CMD then
						-- modify the data
						-- invalidate the data
						idx :=to_integer(unsigned(bus_req_s.adr(3 downto 0)));
						tmp_snp_req.dat(511-idx*16 downto 511-idx*16-32) := tmp_snp_req_s.dat;
					...
					-- set the exclusive bit to 1
						tmp_snp_req.frontinfo(52 downto 52):="0";
					end if;
					writerequest4 <= tmp_snp_req;
		...
		if (reset = '1') then
			...
			if state = 0 then           -- wait_fifo
				...
				if readres.val = '1' then -- if hit
					tmp_up_res := (readres.val, readres.cmd, readres.tag, readres.id, readres.addr, readres.dat(31 downto 0));
					if read_hit = '1' then
	...
	bus_res_p : process(reset, clock)
		variable state     : integer := 0;
		variable tmp_req   : cacheline;
		variable tmp_req_b : BMSG_T;
	begin
		if reset = '1' then
			-- reset signals
			cpu_res2 <= ZERO_MSG;
		...
		if (reset = '1') then
			...
			if st = a then
				if readreq.val = '1' then
					...
					readres <= (readreq.val, readreq.cmd, readreq.tag, readreq.id, readreq.addr, memcont(511 downto 0),memcont(547 downto 512));
					if memcont(547+offset downto 547+offset) = "0" or (readreq.cmd = READ_CMD and memcont(546 downto 546) = "0") 
					or readreq.cmd = WRITE_CMD or memcont(529 downto 512) /= readreq.adr(31 downto 14) then -- 31 to 14
						readhit <= '0';
					else
						readhit <= '1';
					end if;
					...
					if readreq.cmd=WRITE_CMD then
						...
						writereq4 <= (readreq.val, readreq.cmd, readreq.tag, readreq.id, readreq.addr, memcont(511 downto 0),memcont(547 downto 512));
						st := b;
					end if;
				end if;
			elsif st = b then
	...
	begin
		if (reset = '1') then
			...
			if writereq.val = '1' then
					...
					ROM_array(idx)<=writereq.frontinfo&writereq(31 downto 14)&writereq.dat;
					writeack <='1';
			end if;
		end if;
	end process;
